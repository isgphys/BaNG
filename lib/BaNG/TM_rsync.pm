...
sub queue_rsync_backup {
    my ( $host_arg, $group_arg, $host, $group, $initial, $missingonly, $noreport, $taskid ) = @_;
    my $jobid;
    ...
    if (( $hosts{"$host-$group"}->{hostconfig}->{BKP_THREAD_SRCFOLDERS} ) || ( $#src_folders  == 0 )) {
        ...
        if ( $hosts{"$host-$group"}->{hostconfig}->{BKP_THREAD_SUBFOLDERS} ) {
            ...
            foreach my $folder (@src_folders) {
                ...
                _queue_remote_subfolders( $taskid, $jobid, $host, $group, $bkptimestamp, $dosnapshot, $folder );
            }
...
sub _queue_remote_subfolders {
    my ( $taskid, $jobid, $host, $group, $bkptimestamp, $dosnapshot, $srcfolder ) = @_;
