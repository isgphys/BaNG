...
like(   $output,    qr|pre-queue checks for host localhost group $group|            , "Pre_Queue checks for localhost $group"               );
like(   $output,    qr|Check existing target_path for host localhost group $group|  , "Check existing target_path for host localhost $group");
...
like(   $output,    qr|Queueing backup for host localhost group $group|             , "Queueing backup for host localhost $group"           );
like(   $output,    qr|Number of source folders: 1|                                 , "Correct source folders for localhost $group"         );
like(   $output,    qr|Status source folder threading: 0|                           , "Status source folder threading: 0"                   );
like(   $output,    qr|reorder queue by priority|                                   , "reorder queue by priority"                           );
